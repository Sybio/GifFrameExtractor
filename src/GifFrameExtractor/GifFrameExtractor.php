 ...
 * @contributor gpv-dev (Alejandro Hernandez /gpv.dev@gmail.com)
...
class GifFrameExtractor
{
     ...
     */
    private function getApplicationData($first = false)
    {
        ...
        if ($startdata == chr(0x21).chr(0xff)) {
            ...
            $this->pointerForward($this->readByteInt()); // first time skips "NETSCAPE2.0" bytes


            if ($first) { // extract # of loops

                $startLoopSubData = $this->readByte(2);
